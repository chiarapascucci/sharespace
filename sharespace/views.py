"""
    ...
    this file contains functions that classes that handle requests from the client (received as a request object)
    and return responses to the client based on the received request
    request are sent to the right view by Django using the URL paths defined in the urls.py files

    most view will work according to the following pattern:
    - request object is received
    - the view will interpret the request
    - the view will perform data retrieval from the server / data manipulation in the server / data validation
    - the view will then return a rendered template (HTML file) using one of created template in the template folder
...
# this type of view is used for simpler pages in the application
# that do no require substantial data retrieval, manipulation, or validation
...
# -------------------------------------- VIEWS USED TO SERVICE AJAX REQUESTS ------------------------------ #
...
def ajax_cancel_booking(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            loan.delete()
            return JsonResponse(
                {'loan_deleted': True, 'msg': "your booking has been deleted", 'redirect_url': redirect_url})
...
def ajax_delete_item(request, item_slug):
    if request.method == 'POST':
        ...
        return JsonResponse(
            {'item_deleted': False, 'msg': "something went wrong, please try again (wrong request type"})
            ...
            if test_item_ownership(request, item_slug):
                ...
                try:
                    ...
                    if form_is_valid['form_valid']:
                        ...
                        loan = Loan.objects.create(requestor=up, item_on_loan=item, due_date=due_date,
                                                   out_date=out_date)
                        loan.update_loan()
        ...
        try:
            ...
            try:
                ...
                if proposal.proposal_submitter == up:
                    proposal.delete()
                    return JsonResponse({'prop_deleted': True, 'msg': "Your purchase proposal has been deleted",
                                         'redirect_url': redirect_url})
                else:
                    return JsonResponse({'prop_deleted': False, 'msg': "this is not your proposal",
                                         'redirect_url': redirect_url})
            except PurchaseProposal.DoesNotExist:
                return JsonResponse({'prop_deleted': False, 'msg': "No proposal was found, please try again",
                                     'redirect_url': redirect_url})

        except KeyError:
            return JsonResponse(
                {'prop_deleted': False, 'msg': "No proposal was found, please try again", 'redirect_url': redirect_url})
    else:
        return JsonResponse(
            {'prop_deleted': False, 'msg': "Wrong request type, please try again", 'redirect_url': redirect_url})


# -------------------------------- CLASS BASED VIEWS ------------------------------ #
# this type of views are used to handle more complicated pages in the application
...
class AccountDeletionView(View):
    ...
    def post(self, request):
        pp(request.POST)
        return HttpResponse(
            "your account deletion request has been received. You'll get a confirmation email when the request has been processed")
...
class SearchView(View):
    ...
    def get(self, request):
        ...
        up_dict = extract_us_up(request)
        print("views - 700 - log : up item_dict :", up_dict)
        if not up_dict['up'] is None:
...
class LoanCompleteNotificationView(View):
    ...
    def post(self, request, notification_slug):
        ...
        if input_value == 'returned-ok':
            ...
            notification.content_object.mark_as_complete_by_lender()
            return redirect(
                reverse('sharespace:submit_report', kwargs={'subject_slug': notification.content_object.loan_slug}))
...
def unpack_slug_for_report(slug, up):
    ...
    if slug.find("loan") >= 0:
        try:
            ...
            if (loan.requestor == up or loan.item_on_loan.guardian == up) and not (
                    loan.status == loan.ACTIVE or loan.status == loan.FUTURE):
                return loan
            else:
                return None
        except Loan.DoesNotExist:
        ...
        try:
            ...
            try:
                ...
                return {'form_valid': False,
                        'msg': f"no user profile found for item guardian. selected was: {request.POST['guardian-selector-nm']}"}
        except CustomUser.DoesNotExist:
            print("no user")
            return {'form_valid': False,
                    'msg': f"no user found for item guardian. selected was: {request.POST['guardian-selector-nm']}"}
    else:
        up_dict = extract_us_up(request)
        if up_dict['up'] is None:
            return {'form_valid': False, 'msg': 'no user found'}
